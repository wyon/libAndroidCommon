...
public class BackgroundExecutor {
	...
	public static Executor DEFAULT_EXECUTOR = Executors
			.newScheduledThreadPool(2 * Runtime.getRuntime()
					.availableProcessors());
	...
	public static final WrongThreadListener DEFAULT_WRONG_THREAD_LISTENER = new WrongThreadListener() {
		...
		@Override
		public void onWrongBgSerial(String currentSerial,
			...
			throw new IllegalStateException(
					"Method invocation is expected from one of serials "
							+ Arrays.toString(expectedSerials)
							+ ", but it was called from " + currentSerial
							+ " serial");
		}
	};
		...
		if (delay > 0) {
			...
			if (!(executor instanceof ScheduledExecutorService)) {
				throw new IllegalArgumentException(
						"The executor set does not support scheduling");
			}
			ScheduledExecutorService scheduledExecutorService = (ScheduledExecutorService) executor;
			future = scheduledExecutorService.schedule(runnable, delay,
	 ...
	 *             executor)
	 */
	public static void execute(final Runnable runnable, String id, int delay,
			String serial) {
	...
	/**
	 * Check if current thread is a background thread and, optionally, restrict
	 * it with passed serials. If no serials passed and current thread is UI,
	 * then {@link WrongThreadListener#onBgExpected(String...)} will be called.
	 * If current thread is not UI and serials list is empty, then method just
	 * returns. Otherwise, if method was called not during {@link Task}
	 * execution or the task has no serial, then
	 * {@link WrongThreadListener#onWrongBgSerial(String, String...)} will be
	 * called with null for the first parameter. If task has serial but passed
	 * serials don't contain that, then
	 * {@link WrongThreadListener#onWrongBgSerial(String, String...)} will be
	 * called with task's serial for the first parameter.
	 * 
	 * @param serials
	...
	/**
	 * A callback interface to be notified when current thread, in which method
	 * has been invoked, is wrong.
	 * 
